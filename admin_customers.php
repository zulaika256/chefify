...
// Load customers (exclude admins) with points
try {
  $stmt = $pdo->prepare("SELECT u.user_id AS id, u.fullname AS name, u.email, u.role, COALESCE(r.points,0) AS points, COALESCE(u.avatar, 'img/avatar1.jpg') AS avatar FROM users u LEFT JOIN reward_points r ON u.user_id = r.user_id WHERE u.role != 'admin' ORDER BY points DESC, u.user_id ASC");
  $stmt->execute();
...
$customers_json = json_encode($customers, JSON_UNESCAPED_SLASHES);
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
<link rel="stylesheet" href="admin_customers.css">
...
<div class="modal-overlay" id="editModal">
  <div class="modal-content">
    ...
    <select id="editRole" style="width:100%; padding:10px; margin-bottom:15px; border-radius:8px; border:1px solid #ddd;">
...
customers.forEach(c => { if (!('membership' in c)) c.membership = (parseInt(c.points) >= 10000 ? 'VIP' : 'Regular'); });
...
function openEdit(id) {
  ...
  document.getElementById('editRole').value = c.role; // Set current role
...
function saveEdit() {
  ...
  fd.append('membership', newMembership); // This doesn't seem to stay in DB but kept for frontend consistency
  fd.append('role', newRole);
  
  fetch('admin_customer_actions.php', { method: 'POST', body: fd })
    .then(r=>r.json()).then(j=>{
      if (j.success) {
        const c = customers.find(x => x.id === id);
        if (c) { 
            c.name = newName; 
            c.membership = newMembership;
            c.role = newRole;
        }
        closeModals(); 
        renderAll(); 
  ...
  if(!isNaN(amt) && amt > 0) {
    ...
    fetch('admin_customer_actions.php', { method: 'POST', body: fd })
      .then(r=>r.json()).then(j=>{
        if (j.success) {
          const c = customers.find(x => x.id === id);
          if (c) c.points = (parseInt(c.points)||0) + amt;
          closeModals(); renderAll(); showToast(`Gifted ${amt} points to ${c.name}! 🎁`);
        } else alert('Error: ' + j.message);
      }).catch(()=>alert('Network error'));
  }
}
