...
// Load orders with items and customer name
try {
  $sql = "SELECT o.order_id AS id, COALESCE(u.fullname, 'Guest') AS customer, o.total_amount AS total, o.order_status AS status, o.points_earned AS points, o.created_at, o.payment_method,
      IFNULL(GROUP_CONCAT(CONCAT(mi.name, ' x', oi.quantity) SEPARATOR '||'), '') AS items_concat
      FROM orders o
  ...
  foreach ($rows as $r) {
    $items = [];
    if (!empty($r['items_concat'])) {
      $parts = explode('||', $r['items_concat']);
...
$orders_json = json_encode($orders, JSON_UNESCAPED_SLASHES);
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
async function updateStatus(id, newStatus){
  ...
  payload.append('order_id', id);
  payload.append('status', newStatus.toLowerCase());
  try {
    const res = await fetch('admin_order_actions.php', { method:'POST', body: payload });
...
function confirmCancel(){
  ...
  fetch('admin_order_actions.php', { method: 'POST', body: payload })
    .then(r=>r.json()).then(j=>{
      if (j.success) {
        const order = orders.find(o => o.id === id);
        if (order) { order.status = 'Cancelled'; order.note = reason; }
